...
- **Never name a specific tool or technology in the hook.** Not Claude Code, Cursor, Copilot, Figma-to-MCP, React, TypeScript, C#, any of them. Reference the *practice* instead: "AI-assisted development tooling," "AI tooling every day." This supersedes an earlier version of this line that said naming a tool the posting mentions beats the generic AI hook. It doesn't, and it produced hooks like "your values of using claude ai is exactly where I excel." See [jordan-intro-hook-rules.md](jordan-intro-hook-rules.md).
